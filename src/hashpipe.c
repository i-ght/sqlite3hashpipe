...
static int rehash(Hash *pH, unsigned int new_size){
  ...
  */
  const size_t malloc_size = new_size*sizeof(struct _ht);
  new_ht = (struct _ht *)malloc( malloc_size );

  if( new_ht==0 ) return 0;
  free(pH->ht);
  pH->ht = new_ht;
  pH->htsize = new_size = malloc_size / sizeof(struct _ht);
...
*/
void *hash_lookup(const Hash *pH, const char *pKey, int nKey){
...
*/
void *hash_add(Hash *pH, const char *pKey, int nKey, void *data){
